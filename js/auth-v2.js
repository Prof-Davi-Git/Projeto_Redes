/* =========================================================
   ...
   Além das ações antigas, aceita botões com data-auth-action.
   Assim novas áreas podem usar a mesma autenticação sem alterar o núcleo.
   ========================================================= */
...
(() => {
  ...
  function identificarAcaoProtegida(event) {
    ...
    // Extensão genérica para novas áreas do projeto.
    const custom = alvo.closest('[data-auth-action]');
    if (custom) {
      return {
        element: custom,
        action: String(custom.dataset.authAction || 'salvar uma alteração no projeto').trim()
      };
    }
